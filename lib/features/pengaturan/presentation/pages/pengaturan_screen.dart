...
class PengaturanScreen extends ConsumerWidget {
          ...
          children: [
            ...
            _buildSettingsGroup(c, [
              _buildNavItem(
                ...
                subtitle: 'Ketuk untuk mengatur kasir',
                showDivider: false,
                onTap: () {
                  _showManajemenKasirSheet(context, ref);
                },
              ),
            ]),
            const SizedBox(height: 24),
  ...
  void _showManajemenKasirSheet(BuildContext context, WidgetRef ref) {
    final c = AppColors.of(context);
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        FilledButton.icon(
                          onPressed: () {
                            _showTambahKasirDialog(context, ref);
                          },
                    ...
                    Expanded(
                      child: Consumer(
                        builder: (context, ref, child) {
                          ...
                          if (kasirState.isLoading) {
                            return const Center(child: CircularProgressIndicator());
                          }
                          
                          if (kasirState.error != null) {
                            return Center(child: Text(kasirState.error!, style: TextStyle(color: Colors.red)));
                          }

                          // Owner is always at top
                          final ownerName = settings.namaToko.isEmpty ? 'Owner' : settings.namaToko;
                          
                          return ListView.separated(
                            controller: scrollController,
                            itemCount: kasirState.kasirList.length + 1,
                            ...
                            itemBuilder: (_, index) {
                              if (index == 0) {
                                return ListTile(
                                  ...
                                  leading: CircleAvatar(
                                    ...
                                    child: Text(
                                      ownerName.isNotEmpty ? ownerName.substring(0, 1).toUpperCase() : 'O',
                              ...
                              final kasir = kasirState.kasirList[index - 1];
                              return ListTile(
                                contentPadding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                                leading: CircleAvatar(
                                  backgroundColor: c.primaryContainer.withValues(alpha: 0.2),
                                  child: Text(
                                    kasir.nama.isNotEmpty ? kasir.nama.substring(0, 1).toUpperCase() : 'K',
                                    style: TextStyle(color: c.primary, fontWeight: FontWeight.bold),
                                  ),
                                ),
                                title: Text(
                                              ...
                                              child: Column(
                                                ...
                                                children: [
                                                  ListTile(
                                                    ...
                                                    onTap: () {
                                                      Navigator.pop(ctx3);
                                                      ref.read(kasirProvider.notifier).toggleStatus(kasir.id, kasir.aktif);
                                                    },
                                                  ),
                                                  ListTile(
                                                    ...
                                                    onTap: () {
                                                      Navigator.pop(ctx3);
                                                      ref.read(kasirProvider.notifier).hapusKasir(kasir.id);
                                                    },
                                                  ),
                                                ],
                                              ),
  ...
  void _showTambahKasirDialog(BuildContext context, WidgetRef ref) {
    final c = AppColors.of(context);
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: c.surfaceContainerLow,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: Text('Tambah Kasir', style: TextStyle(color: c.onSurface)),
        content: SingleChildScrollView(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
          ...
          Consumer(
            builder: (context, ref, child) {
              ...
              return ElevatedButton(
                ...
                onPressed: kasirState.isLoading
                    ? null
                    : () async {
                        ...
                        if (success) {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text('Kasir berhasil ditambahkan'), backgroundColor: c.primaryGreen),
                          );
                        } else {
                          final error = ref.read(kasirProvider).error;
                          if (context.mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: Text(error ?? 'Gagal menambahkan kasir'), backgroundColor: Colors.red),
                            );
                          }
                        }
                      },
                child: kasirState.isLoading
